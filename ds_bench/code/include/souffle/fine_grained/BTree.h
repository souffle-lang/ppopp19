...
//#ifdef IS_PARALLEL
        ...
        // a pointer to the last lock held
        ...
        // a utility to release the currently held lock
        auto releaseLock = [&]() {
        	if (cur_lock) cur_lock->unlock();
        	cur_lock = nullptr;
        };

        if (hints.last_insert) {
            // lock the indicated node
        	hints.last_insert->lock.lock();
