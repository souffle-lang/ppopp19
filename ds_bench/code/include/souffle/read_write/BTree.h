...
private:
    ...
    typedef ReadWriteLock lock_type;
...
//            assert(!this->parent || this->parent->lock.isLocked());
//            assert((this->parent != nullptr) || root_lock.isLocked());
#endif
...
#ifdef IS_PARALLEL
//                assert(!this->parent || this->parent->lock.isLocked());
//                assert((this->parent) || root_lock.isLocked());
#endif
    ...
    public:
        ...
        void printTree(std::ostream& out, const std::string& prefix) const {
            ...
            if (this->lock.isLocked()) std::cout << " locked";
#endif
...
//#ifdef IS_PARALLEL
    ...
    restart:
        ...
        // a pointer to the last lock the read permission is held
        lock_type* cur_lock = nullptr;

        // a utility to descent locks into the tree
        auto moveLockTo = [&](node* next) {
        	next->lock.start_read();
        	// if the next node is full, it may split
        	if (next->isFull()) {
        		// stick to parent lock
        		next->lock.end_read();
        	} else {
        		// move to this node
        		cur_lock->end_read();
        ...
        if (hints.last_insert) {
            // lock the indicated node
        	hints.last_insert->lock.start_read();
            // check whether it covers the key
        	if (!hints.last_insert->isFull() && covers(hints.last_insert, k)) {
				// use hinted location
            	...
            	// give up the lock
            	hints.last_insert->lock.end_read();
            }
        }
        ...
        if (!cur) {
			...
			root_lock.start_read();
			cur_lock = &root_lock;

			// start with root
			cur = root;

			// acquire lock of the next node to be accessed
			moveLockTo(cur);
        ...
        while (true) {
            ...
            if (cur->inner) {
                ...
                // (potentially) move lock to next node
                moveLockTo(next);
            ...
            if (cur->numElements >= node::maxKeys) {
                ...
                // try upgrade to write
                if (!cur_lock->try_upgrade_to_write()) {
                	// there is a collision => restart
                	cur_lock->end_read();

                	// yield CPU time
                	pthread_yield();

                    // delete hint pointer since insert has to start from the top
                    hints.last_insert = nullptr;
...
//                    return insert(k, hints);
                ...
                // end the write protection
                cur_lock->end_write();

                // update hint pointing to force top-level restart
                hints.last_insert = nullptr;
            ...
            if (!cur_lock->try_upgrade_to_write()) {
                ...
                // update hint pointing to current node
