    ...
    struct node : public base {
        ...
        void split(node** root, lock_type& root_lock, int idx) {
            ...
            for (unsigned i = split_point + 1, j = 0; i < maxKeys; ++i, ++j) {
                //sibling->keys[j] = keys[i].load();
		sibling->keys[j].store(keys[i].load(std::memory_order_relaxed),std::memory_order_relaxed);
            }
        ...
        int rebalance_or_split(node** root, lock_type& root_lock, int idx) {
            ...
            if (parent && pos > 0) {
                ...
                if (num > 0) {
                    std::atomic<Key>* splitter = &(parent->keys[this->position - 1]);

                    // .. move keys to left node
                    left->keys[left->numElements] = splitter->load(std::memory_order_acquire);
                    for (size_type i = 0; i < num - 1; ++i) {
                        //left->keys[left->numElements + 1 + i] = keys[i].load();
			left->keys[left->numElements + 1 + i].store(keys[i].load(std::memory_order_relaxed),std::memory_order_relaxed);
                    }
                    *splitter = keys[num - 1].load(std::memory_order_acquire);

                    // shift keys in this node to the left
                    for (size_type i = 0; i < this->numElements - num; ++i) {
                        //keys[i] = keys[i + num].load();
			keys[i].store(keys[i+num].load(std::memory_order_relaxed),std::memory_order_relaxed);
                    }
    ...
    private:
        ...
        void grow_parent(node** root, lock_type& root_lock, node* sibling) {
            ...
            if (this->parent == nullptr) {
                ...
                new_root->keys[0] = keys[this->numElements].load(std::memory_order_acquire);
		...
		//keys[i + 1] = keys[i].load();
                keys[i+1].store(keys[i].load(std::memory_order_relaxed),std::memory_order_relaxed);
...
public:
    ...
    bool insert(const Key& k, operation_hints& hints) {
        ...
        // test last insert
        typename lock_type::Lease cur_lease;
        ...
        while (true) {
            ...
            if (cur->numElements >= node::maxKeys) {
                ...
                if (((size_type)idx) > cur->numElements) {
                    ...
                    // update hint pointing to parent
                    hints.last_insert = cur->parent;
            ...
            for (int j = cur->numElements; j > idx; --j) {
                // cur->keys[j] = cur->keys[j - 1].load();
		cur->keys[j].store(cur->keys[j - 1].load(std::memory_order_relaxed),std::memory_order_relaxed);
            }

            // insert new element
            cur->keys[idx].store(k,std::memory_order_relaxed);
            cur->numElements++;
...
//        if (empty()) return end();
    ...
    size_type getMemoryUsage() const {
        return sizeof(*this) + ((!root) ? 0 : root->getMemoryUsage());
    }
