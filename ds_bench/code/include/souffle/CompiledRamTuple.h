...
namespace tbb {
	...
	template<typename Domain, std::size_t arity>
    struct tbb_hash<souffle::ram::Tuple<Domain,arity>> {
        tbb_hash() {}
        size_t operator()(const souffle::ram::Tuple<Domain,arity>& k) const {
        	return std::hash<souffle::ram::Tuple<Domain,arity>>()(k);
        }
    };
}
